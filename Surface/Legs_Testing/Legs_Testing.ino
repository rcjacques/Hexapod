...
///////////////////////////////////////////////////////////////////////////////////////
//RF = 0; RM = 1; RB = 2; LF = 3; LM = 4; LB = 5;
void lift_leg(int leg_num){
  switch(leg_num){
    case 0:
...
void for_rest_leg(int leg_num){
  switch(leg_num){
    ...
    case 3:
      ...
      pwm.setPWM(LF_sho, 0, LF_for_rest.sho); //
    break;
    case 4:
    ...
    break;
    case 5:
      pwm.setPWM(LB_foot, 0, LB_for_rest.foo); 
...
//RF = 0; RM = 1; RB = 2; LF = 3; LM = 4; LB = 5;
void back_lift_leg(int leg_num){
  switch(leg_num){
    case 0:
      pwm.setPWM(RF_foot, 0, RF_back_lift.foo); //
      pwm.setPWM(RF_seg, 0, RF_back_lift.seg); 
      pwm.setPWM(RF_sho, 0, RF_back_lift.sho); //
    break;
    case 1:
      pwm.setPWM(RM_foot, 0, RM_back_lift.foo); //
      pwm.setPWM(RM_seg, 0, RM_back_lift.seg); //
      pwm.setPWM(RM_sho, 0, RM_back_lift.sho); //
    break;
    case 2:
      pwm.setPWM(RB_foot, 0, RB_back_lift.foo); //
      pwm.setPWM(RB_seg, 0, RB_back_lift.seg); //
      pwm.setPWM(RB_sho, 0, RB_back_lift.sho); //
    break;
    case 3:
      pwm.setPWM(LF_foot, 0, LF_back_lift.foo); //
      pwm.setPWM(LF_seg, 0, LF_back_lift.seg); //
      pwm.setPWM(LF_sho, 0, LF_back_lift.sho); //
    break;
    case 4:
      pwm.setPWM(LM_foot, 0, LM_back_lift.foo); 
      pwm.setPWM(LM_seg, 0, LM_back_lift.seg); 
      pwm.setPWM(LM_sho, 0, LM_back_lift.sho); //
    break;
    case 5:
      pwm.setPWM(LB_foot, 0, LB_back_lift.foo); 
      LB_seg.write(LB_back_lift.seg);
      LB_sho.write(LB_back_lift.sho);
    break;
    default:
      //do nothing
    break;
...
//RF = 0; RM = 1; RB = 2; LF = 3; LM = 4; LB = 5;
void back_rest_leg(int leg_num){
  switch(leg_num){
    case 0:
...
//RF = 0; RM = 1; RB = 2; LF = 3; LM = 4; LB = 5;
void for_step_leg(int leg_num){ 
  int pause = 200;
  lift_leg(leg_num);
  delay(pause);
  for_lift_leg(leg_num);
  delay(pause);
  for_rest_leg(leg_num);
  delay(pause);
}

//RF = 0; RM = 1; RB = 2; LF = 3; LM = 4; LB = 5;
void wave_legs(int num_waves){ 
  int pause = 150;
...
void walk_forward_creep(int num_creeps){
  for(int count = 0; count < num_creeps; count++){
    for(int i = 0; i < 6; i++){
      for_step_leg(i);
    }
    for(int j = 0; j < 6; j++){
...
void loop() {
  all_rest();
  walk_forward_creep(5);
  //wave_legs(10);
//  walk_forward_creep(1);
